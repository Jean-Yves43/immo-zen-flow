import { Users, Home, CreditCard, Wrench, ArrowUpRight } from "lucide-react";
    ...
    <div className="space-y-6 animate-fade-in">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-1">Bienvenue, Marc. Voici un aperçu de votre portefeuille.</p>
        </div>
        <Button className="bg-secondary hover:bg-secondary/90 text-secondary-foreground">
          <Users className="mr-2 h-4 w-4" />Ajouter un propriétaire
        </Button>
      </div>
          ...
          <Card key={card.title} className="hover-lift">
            ...
            <CardContent>
              ...
              <div className="flex items-center gap-1 mt-1">
                ...
                <span className="text-xs text-secondary font-medium">{card.change}</span>
              </div>
            </CardContent>
          </Card>
        ))}
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <Card>
          <CardHeader><CardTitle>Statut des biens</CardTitle></CardHeader>
          <CardContent>
            <ChartContainer config={{ value: { label: "Biens" } }} className="h-[250px]">
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie data={propertyStatus} cx="50%" cy="50%" innerRadius={60} outerRadius={100} paddingAngle={5} dataKey="value">
                    {propertyStatus.map((entry, index) => (<Cell key={`cell-${index}`} fill={entry.color} />))}
                  </Pie>
                  <ChartTooltip content={<ChartTooltipContent />} />
                </PieChart>
              </ResponsiveContainer>
        ...
        <CardContent>
          <div className="space-y-3">
            {recentAlerts.map((alert, index) => (
              <div key={index} className={`flex items-center justify-between p-3 rounded-lg ${alert.urgent ? "bg-destructive/10" : "bg-muted/50"}`}>
