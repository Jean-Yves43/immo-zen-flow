      ...
      <Tabs defaultValue="monthly" className="space-y-6">
        ...
        <TabsContent value="monthly" className="space-y-6">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  ...
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={monthlyData}>
                      ...
                      <Tooltip
                        ...
                        formatter={(value: number) => [`€${value.toLocaleString()}`, "Paiements"]}
                      />
                      <Bar dataKey="paiements" fill="url(#colorGradient)" radius={[4, 4, 0, 0]} />
                      <defs>
            ...
            {/* Tenants & Properties Growth */}
            <Card className="bg-slate-800/50 border-slate-700/50">
              <CardHeader>
                <CardTitle className="text-white flex items-center gap-2">
                  <TrendingUp className="w-5 h-5 text-blue-400" />
                  Croissance Locataires & Propriétés
                </CardTitle>
              </CardHeader>
              <CardContent>
                <div className="h-80">
                  <ResponsiveContainer width="100%" height="100%">
                    <LineChart data={monthlyData}>
                      ...
                      <Tooltip
                        contentStyle={{ backgroundColor: "#1e293b", border: "1px solid #334155", borderRadius: "8px" }}
                      />
                      <Line type="monotone" dataKey="locataires" stroke="#3b82f6" strokeWidth={2} dot={{ fill: "#3b82f6" }} />
                      <Line type="monotone" dataKey="proprietes" stroke="#8b5cf6" strokeWidth={2} dot={{ fill: "#8b5cf6" }} />
                    </LineChart>
                  </ResponsiveContainer>
                </div>
                <div className="flex justify-center gap-8 mt-4">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm text-slate-400">Locataires</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <div className="w-3 h-3 rounded-full bg-purple-500" />
                    <span className="text-sm text-slate-400">Propriétés</span>
                  </div>
                </div>
              </CardContent>
            </Card>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <Card className="bg-slate-800/50 border-slate-700/50">
              <CardHeader>
                <CardTitle className="text-white">Répartition des Moyens de Paiement</CardTitle>
              </CardHeader>
              <CardContent>
            ...
            <Card className="bg-slate-800/50 border-slate-700/50">
              <CardHeader>
                <CardTitle className="text-white">Statut des Paiements</CardTitle>
              </CardHeader>
              <CardContent>
                <div className="h-64 flex items-center justify-center">
                  <ResponsiveContainer width="100%" height="100%">
            ...
            <CardContent>
              ...
              <div className="flex justify-center gap-8 mt-4">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm text-slate-400">Revenus</span>
                </div>
                <div className="flex items-center gap-2">
                  <div className="w-3 h-3 rounded-full bg-red-500" />
                  <span className="text-sm text-slate-400">Dépenses</span>
                </div>
              </div>
            </CardContent>
          </Card>
            ...
            {yearlyData.map((year, index) => (
              <Card key={index} className="bg-slate-800/50 border-slate-700/50">
                <CardContent className="p-4 text-center">
                  <p className="text-lg font-bold text-white mb-2">{year.year}</p>
                  <p className="text-emerald-400 font-semibold">€{(year.revenus / 1000000).toFixed(2)}M</p>
                  <p className="text-xs text-slate-400">Revenus</p>
                  <p className="text-purple-400 font-semibold mt-2">€{(year.benefice / 1000000).toFixed(2)}M</p>
                  <p className="text-xs text-slate-400">Bénéfice</p>
                </CardContent>
              </Card>
            ))}
          </div>
        ...
        <TabsContent value="global" className="space-y-6">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <Card className="bg-slate-800/50 border-slate-700/50 lg:col-span-2">
              <CardHeader>
                <CardTitle className="text-white">Résumé Global de la Plateforme</CardTitle>
              </CardHeader>
              <CardContent>
                <div className="grid grid-cols-2 gap-6">
                  <div className="p-6 rounded-xl bg-gradient-to-br from-emerald-500/20 to-cyan-500/20 border border-emerald-500/30">
                    <p className="text-sm text-emerald-400 mb-2">Total Revenus (All Time)</p>
                    <p className="text-4xl font-bold text-white">€9,990,000</p>
                    <p className="text-sm text-slate-400 mt-2">Depuis 2020</p>
                  </div>
                  <div className="p-6 rounded-xl bg-gradient-to-br from-blue-500/20 to-purple-500/20 border border-blue-500/30">
                    <p className="text-sm text-blue-400 mb-2">Transactions Totales</p>
                    <p className="text-4xl font-bold text-white">45,892</p>
                    <p className="text-sm text-slate-400 mt-2">Paiements traités</p>
                  </div>
                  <div className="p-6 rounded-xl bg-gradient-to-br from-purple-500/20 to-violet-500/20 border border-purple-500/30">
                    <p className="text-sm text-purple-400 mb-2">Taux de Recouvrement</p>
                    <p className="text-4xl font-bold text-white">93%</p>
                    <p className="text-sm text-slate-400 mt-2">Paiements à temps</p>
                  </div>
                  <div className="p-6 rounded-xl bg-gradient-to-br from-orange-500/20 to-amber-500/20 border border-orange-500/30">
                    <p className="text-sm text-orange-400 mb-2">Croissance Annuelle</p>
                    <p className="text-4xl font-bold text-white">+23%</p>
                    <p className="text-sm text-slate-400 mt-2">Par rapport à 2023</p>
                  </div>
                </div>
              </CardContent>
            </Card>
            ...
            <Card className="bg-slate-800/50 border-slate-700/50">
              <CardHeader>
                <CardTitle className="text-white">Indicateurs Clés</CardTitle>
              </CardHeader>
              <CardContent className="space-y-4">
                ...
                <div className="flex items-center justify-between p-3 rounded-lg bg-slate-900/50">
                  <span className="text-slate-400">Loyer moyen</span>
                  <span className="font-bold text-white">€1,250</span>
                </div>
                <div className="flex items-center justify-between p-3 rounded-lg bg-slate-900/50">
                  <span className="text-slate-400">Délai moyen paiement</span>
                  <span className="font-bold text-white">3.2 jours</span>
                </div>
                <div className="flex items-center justify-between p-3 rounded-lg bg-slate-900/50">
                  <span className="text-slate-400">Satisfaction clients</span>
                  <span className="font-bold text-emerald-400">4.8/5</span>
                </div>
                <div className="flex items-center justify-between p-3 rounded-lg bg-slate-900/50">
                  <span className="text-slate-400">Temps réponse support</span>
                  <span className="font-bold text-white">2.4h</span>
                </div>
              </CardContent>
            </Card>
          </div>
